...
def main(args):
    ...
    if args.precision == "W16A16":
        ...
        #model.llm = load_awq_model(model.llm, args.quant_path, 4, 64, args.device)
        print("3")
    ...
    while True:
        ...
        #input_prompt = "Is there a person and is the person drinking enough beer? Describe in one sentence."
        ...
        #input_prompt = "Describe in a few words."
        input_prompt = "Describe in one sentence."
        print("-" * 50)
        if input_prompt == "":
            print("EXIT...")
            time_stats.show()
        ...
        # Wait for sound to be played first:
        sound_file = "example.wav"
        while os.path.exists(sound_file):
            time.sleep(0.1)
            print("Waiting...")

        os.environ["SUNO_USE_SMALL_MODELS"] = "True"
        subprocess.run([
            "python3", "-m", "bark", "--text", outputs, "--history_prompt", "v2/en_speaker_2", "--output_filename", sound_file,
        ])
